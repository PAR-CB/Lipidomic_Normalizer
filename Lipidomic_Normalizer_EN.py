...
def movefilestogroups(classsamples):
    ...
    zipfilename=tk.simpledialog.askstring(title='Info',prompt='Write the name of the *.zip file where\n\
                                               the *.csv for MetaboAnalyst will be saved.',initialvalue='trial')
...
tk.messagebox.showinfo(title='Lipidomic_Normalizer', message=f"This program transforms a *.xlsx file (Excel created by 'Template.xlsx') to a proper format for MetaboAnalyst and Mass Profiler Professional, conducting a normalisation by lipids familys in the process.\n")
...
LipidData=pd.read_excel('./AppData/DatosExcel/LipidClasses.xlsx')
tk.messagebox.showinfo(title='Lipidomic_Normalizer', message='Select what Excel file (*.xlsx), created by\n\
                        "Template.xslx", do you want to open.')
tk.messagebox.showwarning(title='Warning', message='It is recommended that the samples show unique names for their distinction.\n\
                            It is recommended the revision and ellimination with Mass Profinder of the compounds with duplicated names.\n\
                            for the correct running of MetaboAnalyst.\n\
                            Duplication can be of the following types:\n\
                            - Double extraction (same m/z and same retention time RT): it would be elliminated.\n\
                            - Different adducts (similar m/z, different RT): the least relevant adduct would be elliminated.\n\
                            - Isomers(exceptional case of similar masses and RT): both would be taken into account but with different subindexes.\n\
                            In this version, all duplicates would be treated as different compounds. \n\
                            Care must be taken that the lipids names match with the naming code from the file AppData/DatosExcel/LipidClasses.xlsx')
...
if concentrations.equals(controlconcentrations):
    tk.messagebox.showinfo(title='Info', message='"Patrones obtenidos" ISDs \n\
                        matches "Concentraciones añadidas"')
else:
    tk.messagebox.showerror(title='Error', message='"Patrones obtenidos ISDs "\n\
                        DO NOT match with "Concentraciones añadidas". "Concentraciones patrones" have been selected')
...
# WorkDir
tk.messagebox.showinfo(title='Info', message='Choose the directory where you want to save the files.')
workdir=filedialog.askdirectory()
os.chdir(workdir)
# curation of data for elliminating doubles is BEFORE RECEIVING EXCEL
xlsx2csvfilename=tk.simpledialog.askstring(title='Info',prompt='Input the name of the *.csv file where the \n\
                                            Excel data would be saved for both MetaboAnalyst and Mass Profiler Pro.',initialvalue='trial')
...
tk.messagebox.showinfo(title='Info', message=f'{xlsx2csvfilename}.csv, {xlsx2csvfilename}_mpp.csv and {xlsx2csvfilename}_transposed.csv created. Finished job.')
